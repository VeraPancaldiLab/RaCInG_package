...
Both `compute_racing_kernel()` and `compute_racing_montecarlo()` also accept
`communication_type` as a vector (e.g. `c("D", "W", "TT", "GSCC")`), extracting every
requested feature family from the same computed kernel / same simulated graphs in one
call, instead of repeating the expensive step per type. `compute_racing_montecarlo()`
additionally takes `ncores` to parallelize independent patients across cores.
